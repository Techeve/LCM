...
Zentrales Management-Tool für Linux-Server: **einmal installieren, beliebig viele Server remote über SSH verwalten**. Go-Backend (Fiber v3, GORM, SQLite) + Svelte-5-Frontend (Vite, Bootstrap 5) in **einem einzigen Binary** - lauffähig unter Linux, Windows und macOS, ohne externe Abhängigkeiten auf dem Zielsystem.
...
Die vollständige Dokumentation liegt als **reines Markdown** (zweisprachig
Deutsch/Englisch) im Verzeichnis [`docs/`](docs/) - Deutsch in der Wurzel,
Englisch unter [`docs/en/`](docs/en/). Das Repo enthält bewusst **kein**
Astro/Node/npm-Gerüst mehr: Die CI baut die Seiten mit dem zentralen
